...
def cart(request):
    ...
    for i in conn.hkeys(cart_key):
        logger.info(str(type(i)) + str(conn.hget(cart_key, i)) +
                    '值' + str(type(conn.hget(cart_key, i))))
...
def add(request):
    ...
    if num > good.stock:
        # 超过库存
        return JsonResponse(
            {'response': 3, 'error': '商品库存不足，只有' + str(good.stock)})
    # 更新购物车中这个商品的数量
    ...
    return JsonResponse(
        {'response': 5, 'cart_count': cart_count, 'error': '添加购物车记录成功'})
...
def edit(request):
    ...
    if num > good.stock:
        logger.info('数量超过库存')
        return JsonResponse(
            {'res': 3, 'error': '数量已超过库存,现有库存' + str(good.stock) + '件'})
